...
public class EditorView<T, ID extends Serializable> extends GenericView<T> {
	...
	public static final List<String> systemProperties = Arrays.asList( "id", "version", "fullId", "uid", "modifiedTs", "createdTs" );
	...
	protected Button[] buttons = new Button[]{
			...
			new Button("Cancel", new Button.ClickListener() {
				@Override
				public void buttonClick(ClickEvent event) { cancel(); }
			}),
			new Button("Delete", new Button.ClickListener() {
				@Override
				public void buttonClick(ClickEvent event) { delete(); }
			})
	};
	...
	public void newInstance() {
		setInstance(storeService.newInstance());
	}
	
	public void loadInstance(ID id) {
		setInstance(storeService.load(id));
	}
	...
	public void setInstance(T t) {	   
	   ...
	   for (Object propertyId : properties) {
	      // build fields for them and add the fields to this UI
	   	...
	   	f.setReadOnly(systemProperties.contains(propertyId));
	...
	public void save() throws FieldGroup.CommitException {
		...
		storeService.save(fieldGroup.getItemDataSource().getBean());
